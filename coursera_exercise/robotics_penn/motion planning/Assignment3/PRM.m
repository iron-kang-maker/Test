...
for i = 2:nsamples
    ...
    %
    dist_sort = sort(distances);

    k_2 = size(dist_sort, 2);
    if k <= k_2
        k_2 = k;
    end
    dist_sort = dist_sort(1,1:k_2);
    
    for l = 1:k_2
        j = find(distances == dist_sort(l));
        x2 = samples(:, j);
        if LocalPlanner(x, x2)
            nedges = nedges + 1;
            edges(nedges, :) = [i j]; 
            edge_lengths(nedges) = dist_sort(l);
        end
    end
