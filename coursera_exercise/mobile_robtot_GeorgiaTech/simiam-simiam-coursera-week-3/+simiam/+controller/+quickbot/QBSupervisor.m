...
%             fprintf('current_pose: (%0.3f,%0.3f,%0.3f)\n', x, y, theta);
        ...
        function [vel_r, vel_l] = ensure_w(obj, robot, v, w)
            ...
            %fprintf('<<<<robot max lr: %f>>>>>\n', vel_rl_max);
            %fprintf('<<<<robot min lr: %f>>>>>\n', vel_rl_min);
            if (vel_rl_max > obj.robot.max_vel)
                %fprintf('max MMMM\n');
                vel_r = vel_r_d - 1*(vel_rl_max - obj.robot.max_vel);
                vel_l = vel_l_d - 1*(vel_rl_max - obj.robot.max_vel);
            elseif (vel_rl_min < -obj.robot.max_vel)
                %fprintf('min mmmm\n');
                vel_r = vel_r_d - 1*(vel_rl_min + obj.robot.max_vel);
                vel_l = vel_l_d - 1*(vel_rl_min + obj.robot.max_vel);
            else
                %fprintf('others oooo\n');
                vel_r = vel_r_d;
                vel_l = vel_l_d;
